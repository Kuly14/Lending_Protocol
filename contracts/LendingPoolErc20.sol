...
contract LendingPoolErc20 is AlphaToken, ReentrancyGuard {
    ...
    function _updateExchangeRate() private {
        if (totalSupply() == 0) {} else {
            exchangeRate =
                ((_availibleCash() * 1e18) + _totalBorrows() - _getReserves()) /
                totalSupply();
    ...
    function withdraw(uint _amountInATokens) public nonReentrant {
        require(
            balanceOf(msg.sender) >= _amountInATokens,
    ...
    function calculateUtilizationRate() internal view returns (uint) {
        ...
        if (totalBorrows == 0) {
            ...
            uint URate = (totalBorrows * 1e18) /
                (_availibleCash() + totalBorrows - totalReserves);
            return URate;
        }
    }
    ...
    function calculateBorrowRate() internal view returns (uint) {
        if (calculateUtilizationRate() == 0) {
            ...
            uint bRate = baseRate +
                (baseSlope * calculateUtilizationRate()) /
                1e18;
            return bRate;
        } else if (calculateUtilizationRate() > utilizationRateKink) {
            uint ur = calculateUtilizationRate();
            uint overUR = ur - utilizationRateKink;
            uint rateOverKink = baseRate +
                ((baseSlope * utilizationRateKink) / 1e18) +
                ((overUR * slopeAfterKink) / 1e18);
            return rateOverKink;
        }
    }

    function calculateSupplyRate() public view returns (uint) {
        uint supplyRate = (calculateUtilizationRate() * calculateBorrowRate()) /
            1e18;
        return supplyRate;
    }

    function borrow(uint _amount) public nonReentrant {
        // _amount is the amount of tokens he wants to borrow
        require(
            borrowers[msg.sender].numOfLoans == 0,
            "You can only take out 1 loan"
        );
        require(_amount > 0, "The amount can't be 0");
        uint balanceOfSender = (balanceOf(msg.sender)); // Works
        uint necessaryLtv = getLtv(_amount);
        require(
            balanceOfSender >= necessaryLtv,
            "Your balance isn't sufficient to pay the collateral"
        );
    ...
    function repay() public nonReentrant {
        require(
            borrowers[msg.sender].numOfLoans == 1,
            "You don't have any tokens borrowed"
        );
    ...
    function _calculateInterestOfBorrower() internal view returns (uint) {
        // change to internal after testing
        ...
        uint interestInTokens = (borrowers[msg.sender].borrowedTokens *
            interestForTheCaller) / 1e18;
        uint amountToPayBack = interestInTokens +
            borrowers[msg.sender].borrowedTokens;
